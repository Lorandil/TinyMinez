...
#ifdef USE_KEYBOARD_INPUT
  handleKeys();
  auto key = actionKey2Pressed;
  actionKey2Pressed = false;
  return( key );
#else
  #ifdef _USE_ARDUBOY2_
    return( arduboy.pressed( B_BUTTON ) );
  #else
    // sadly the TinyJoypad has no 2nd fire button
    return( false );
  #endif
#endif
}
...
{
  while( isLeftPressed() || isRightPressed() || isUpPressed() || isDownPressed() || isFirePressed() || isFire2Pressed() );
}
